...
const BENIGN = [/already enabled/i, /already installed/i, /already on disk/i, /no updates/i];
...
function runClaude(argv, { optional = false } = {}) {
  ...
  throw new Error(out || 'claude command failed');
}
...
export function installClaudePlugin() {
  ...
  if (!isPluginInstalled()) {
    ...
    runClaude(['plugin', 'update', PLUGIN_ID, '--scope', 'user'], { optional: true });
  }
  if (!isPluginInstalled()) {
    throw new Error(
  ...
  runClaude(['plugin', 'enable', PLUGIN_ID, '--scope', 'user'], { optional: true });
  const settings = readSettings();
